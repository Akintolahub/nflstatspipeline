...
class database: #perform vague actions
	...
	def write(info):
		if response.status_code == 200:
			...
			Jsonfile = open(sanfran_stats, "w").write(writtenInfo)
			print("49ers Stats loaded successfully")
			return Jsonfile
		else:
			print("Error with the pulling from API")
		return

	# def write(info):
	# 	try:
	# 		with open(sanfran_stats, "w") as Jsonfile:
	# 			writtenInfo = json.dumps(info, indent=4)
	...
	def read():
		Jsonfile = open(f"{sanfran_stats}", "r")
